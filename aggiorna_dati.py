...
def get(url):
    req = urllib.request.Request(url, headers={"User-Agent": "btcquant-bot/1.0"})
...
# --- prezzo BTC (CoinGecko) ---
try:
    d = get("https://api.coingecko.com/api/v3/simple/price?ids=bitcoin&vs_currencies=usd")
    dati["price"] = float(d["bitcoin"]["usd"])
except Exception as e:
    errori.append(f"price: {e}")
...
# --- scrittura ---
if errori:
    print("Errori:", "; ".join(errori), file=sys.stderr)
if len(dati) <= 1:
    sys.exit("Nessun dato recuperato, data.json non aggiornato.")
